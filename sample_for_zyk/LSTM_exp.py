...
from models.LSTMAE import train, test, LSTMAE
...
writer = SummaryWriter(log_dir="./log/" + note + starttime,
                       comment=starttime, flush_secs=60)
...
train_dataset, test_dataset = get_concat_ChinaMobileDataset(
    win_len=win_len, data_path=data_path, label_path=label_path)
test_entities = get_test_by_entity(
    win_len=win_len, data_path=data_path, label_path=label_path)
feedback_dataset = get_feedback_data(
    win_len=win_len, data_path=data_path, label_path=label_path)


train_loader = DataLoader(
    train_dataset, batch_size=batch_size, shuffle=True, drop_last=False)
test_loaders = [DataLoader(i, batch_size=batch_size,
                           shuffle=False, drop_last=False) for i in test_entities]


# ======================= Train ================================
model = train(train_loader, params, writer,
              test_loaders=test_loaders)  # type: LSTM
torch.save(model.state_dict(), "./saved_models/LSTMAE.pt")
...
model.load_state_dict(torch.load("./saved_models/LSTMAE.pt"))
...
fine_tune_loader = DataLoader(
    feedback_dataset, batch_size=batch_size, shuffle=True, drop_last=False)

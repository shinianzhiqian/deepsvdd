...
def evaluate_by_entity(model, entity_dataloaders, test_fn, params):
    delay = params.get("delay")
    
    res_ls = []
    for loader in entity_dataloaders:
        raw_seq, est_seq, _, labels = test_fn(model, loader, params)

        scores = np.mean(np.abs(raw_seq - est_seq), axis=1)

        assert len(scores) == len(labels)

        point_res = best_f1_score_point(labels, scores)
        range_res = best_f1_score_with_point_adjust(labels, scores, delay=delay)

        res_ls.append([point_res, range_res])
